...
if __name__ == '__main__':
    ...
    train_reviews = data[3]  # type = list of `reviews`( = list of `sentences`( = list of word tokens) )
    # data[4] = None in Rotten-Tommatoes dataset (no unlabeled train).
    ...
    print "train..."
    new_train_reviews = []  # list of reviews. each review is list of numbers.
    for review in train_reviews:
        words = []  # list of numbers.
        for sentence in review:
            for w in sentence:
                words.append(word2idx[w])
        new_train_reviews.append(np.asarray(words))
    print "test..."
    ...
    data = (data[0], data[1], data[2], new_train_reviews, data[4], new_test_reviews)
    f = open(
