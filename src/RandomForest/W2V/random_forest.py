...
def random_forest_on_w2v(dataset, extend_data=False, embedding_size=300, forest_size=100):
    """
    Train a random forestc classifier with Word2Vec features.
    :param dataset: string to decide which dataset to load (imdb or rot-tom).
    :param extend_data: flag to decide to load the extended data (imdb + rot-tom)
    :param embedding_size: int size of word embedding vector (number of features).
    ...
    print "\nGet training reviews' embedding..."
    ...
    for i, review in enumerate(labeled_train_reviews):
        emb, skip, total = review_to_embedding(review, model)
        train_data_features[i] = emb
        nskipped += skip
        nwords += total
    print train_data_features
    print train_data_features.shape
    print "skipped %d/%d (%f%%) word embeddings." % (nskipped, nwords, nskipped*100./nwords)

    print "\nGet testing reviews' embedding..."
    ...
    w2v_model_file_name = w2v_model_file_name.replace('./W2VModels/', '')
    w2v_model_file_name = w2v_model_file_name.replace(dataset+'_', '')
    ...
    file_name = "./ModelResponses/%s_forest%d_%s_%spredictions.csv"\
...
if __name__ == '__main__':
    ...
    print "\nRunning RandomForest on Word2Vec features"
